...
void main() {
    ...
    //
    // Порог сжат в [0, 1 - shadowPower], и это не косметика. Блок гаснет за
    // shadowPower после отрыва, значит последний блок должен оторваться не
    // позже, чем за столько же до конца шкалы. Без сжатия блок с порогом 1.0
    // доживал до конца анимации с альфой 0.7 — и в этот момент фейдаут просто
    // уничтожался, отчего хвост пикселей исчезал скачком.
    float centerY = (blockPos.y + blk * 0.5) / max(win.y, 1.0);
    float thrMax  = max(0.05, 1.0 - max(shadowPower, 0.05));
    float thr     = mix(hash12(b), clamp(centerY, 0.0, 1.0), thick) * thrMax;
    // brightness > 1 — опережение: всё догорает при time = 1/brightness, а
    // остаток фейдаута идёт по пустому экрану. Это запас на то, что фейдаут
    // уничтожается чуть раньше, чем альфа доходит до нуля.
